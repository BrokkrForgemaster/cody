...
export function LogoSmokeIntro({onComplete}: { onComplete: () => void }) {
    ...
    useEffect(() => {
        ...
        if (!rootRaw || !canvas || !logoRaw) return;
        ...
        function frame(now: number) {
            ...
            // Advance phase so logo-out fires on schedule
            if (t >= 2100 && phase === 1) {
                phase = 2;
            }

            // Logo out — slower, smoother cinematic fade
            if (t >= 3300 && phase === 2) {
                phase = 3;
                ...
                logo.style.transform = "scale(1.045)";
            }

// Overlay out — wait for the logo fade to mostly finish
            if (t >= 4600 && phase === 3) {
                phase = 4;
                root.style.transition = "opacity 0.9s cubic-bezier(0.4, 0, 0.2, 1)";
